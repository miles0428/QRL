...
Observation: [⟨σ_x⟩, ⟨σ_y⟩, Δ⟨σ_x⟩, Δ⟨σ_y⟩] — 4D vector
...
class QuantumSpinCartPoleEnv(gym.Env):
    ...
    Observation (9D, sxy_prevact format):
        o_t = [⟨σ_x⟩_t, ⟨σ_y⟩_t, Δ⟨σ_x⟩_t, Δ⟨σ_y⟩_t, a_0, a_1, a_2, a_3, a_4]
        (4 spin dims + 5 one-hot action dims)
    ...
    def __init__(
        ...
        # Previous action for sxy_prevact observation format (one-hot, 5 dims)
        self._prev_action: int = 4  # IDLE as default
    ...
    def _build_obs(
        ...
        """
        Build the 9D observation vector (sxy_prevact format).

        Args:
            sx: Current ⟨σ_x⟩.
            sy: Current ⟨σ_y⟩.
            sz: Current ⟨σ_z⟩ (unused, kept for signature compatibility).
            prev_sx: Previous ⟨σ_x⟩.
            prev_sy: Previous ⟨σ_y⟩.
            prev_sz: Previous ⟨σ_z⟩ (unused, kept for signature compatibility).
            prev_action: Previous action index [0-4], one-hot encoded.

        Returns:
            np.ndarray: 9D observation [sx, sy, Δsx, Δsy, a_0, a_1, a_2, a_3, a_4].
        """
        # One-hot action encoding
        one_hot = np.zeros(5, dtype=np.float32)
        one_hot[prev_action] = 1.0
        return np.concatenate([
            np.array([sx, sy, sx - prev_sx, sy - prev_sy], dtype=np.float32),
            one_hot,
        ])
    ...
    def reset(self, seed: Optional[int] = None, options=None):
        ...
        obs = self._build_obs(sx0, sy0, sz0, sx0, sy0, sz0, self._prev_action)  # Δsx=0, Δsy=0
        info = {
            "episode_step": 0,
            "sx": sx0,
            "sy": sy0,
    ...
    def step(self, action: int):
        ...
        obs = self._build_obs(sx, sy, sz, self._prev_sx, self._prev_sy, self._prev_sz, self._prev_action)
