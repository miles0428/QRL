...
class TorchStatevectorQNN(torch.nn.Module):
    """QNN wrapper exposing the torch-statevector backend as a callable module.

    This is the torch_sv backend for CircuitBackend. It wraps compile_circuit,
    observable_matrices, and simulate into a torch nn.Module interface:
      - forward(encoded) -> (B, n_obs) expectation values
      - .weights  -> nn.Parameter of circuit variational angles
    """
    ...
    def __init__(
        ...
        super().__init__()
        # Compile the circuit into an executable form (once, at init)
        self._compiled = compile_circuit(
            circuit, list(input_params), list(weight_params)
        )
        # Observable matrices as a buffer (constant, moves with .to()/.cuda())
        obs_mats = observable_matrices(observables, circuit.num_qubits)
        self.register_buffer("_obs", obs_mats)
        n_w = len(weight_params)
        if initial_weights is None:
            initial_weights = torch.zeros(n_w, dtype=torch.float32)
        self.weights = torch.nn.Parameter(initial_weights.float())

    def forward(self, encoded: torch.Tensor) -> torch.Tensor:
        """Run the circuit on encoded inputs; returns (B, n_obs)."""
        return simulate(self._compiled, encoded, self.weights, self._obs)
