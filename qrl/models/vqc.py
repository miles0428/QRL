...
class _TorchStatevectorBackend(nn.Module):
    ...
    def __init__(self, circuit, input_params, weight_params, observables, seed=None):
        super().__init__()
        from qrl.backends import torch_statevector as tsv
...
class VQCQFunction(BaseVQC):
    def __init__(
        ...
        # Do NOT call any __init__ here.  The MRO chain (VQCQFunction ->
        # BaseVQC -> QFunction -> nn.Module) guarantees nn.Module.__init__()
        # is called exactly once: BaseVQC.__init__() -> super().__init__() ->
        # QFunction.__init__() [no-op] -> nn.Module.__init__().  If we called
        # object.__init__(self) first, __dict__ would be created before
        # _parameters, and nn.Module.__setattr__'s hasattr(self,'__dict__')
        # guard would bypass _parameters registration entirely, leaving lam/w/vqc
        # invisible to optimizer.parameters().
        #
        # Build Parameters first (they are plain Python objects), then assign
        # them via nn.Module.__setattr__ AFTER BaseVQC.__init__() completes.
        lam = nn.Parameter(torch.ones(len(input_params)))
        w = nn.Parameter(torch.ones(n_actions))

        # Initialize BaseVQC -- this is where nn.Module.__init__() runs
        BaseVQC.__init__(
            self,
            vqc=vqc_backend,
            lam=lam,
            preprocess_observations=self._preprocess_observations,
            head_transform=self._head_transform,
        )

        # Now _parameters exists and nn.Module.__setattr__ hook is active.
        # Assign via direct setattr to trigger the hook and register in _parameters.
        self.lam = lam
        self.w = w

        # Remaining instance attributes (plain Python, not nn.Module managed)
    ...
    def _preprocess_observations(self, states: torch.Tensor) -> torch.Tensor:
        """Apply normalize_observation and per_layer_encoding tiling."""
    ...
    def _head_transform(self, expvals: torch.Tensor) -> torch.Tensor:
        """Apply output rescaling ([-1,1]->[0,1]) and w scaling."""
        if self.output_rescaling:
            expvals = (expvals + 1.0) / 2.0
        return expvals * self.w
