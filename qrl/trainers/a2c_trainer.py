...
from qrl.trainers.pg_trainer import _beta_to_str, _collect_round
from qrl.trainers.seeds import set_seed
from qrl.trainers.trainer_base import _param_to_str

# A2C does NOT use an exploration schedule, so epsilon is omitted from the header.
# The full set of columns is: episode, total_env_steps, episode_reward, avg_reward_100,
# mean_loss, wall_clock_s, grad_steps, w, lam, eval_mean_reward, eval_std_reward,
# entropy, beta, value_loss, explained_variance.
A2C_CSV_HEADER = [
    "episode",
    "total_env_steps",
    "episode_reward",
...
if __name__ == "__main__":
    ...
    result = train_a2c(
        actor, critic, optimizer, results_path="results/_a2c_smoke.csv",
        checkpoint_path="results/_a2c_smoke.pt", seed=0,
    ...
    assert "epsilon" not in df.columns, "epsilon should not exist in A2C CSV"
    import os as _os
    _os.remove("results/_a2c_smoke.csv")
    _os.remove("results/_a2c_smoke.pt")
